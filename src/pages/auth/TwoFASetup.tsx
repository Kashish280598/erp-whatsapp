...
import { reInitiateMFASetup, setOTPCode, verifyMFASetup } from "@/lib/features/auth/authSlice";
...
const TwoFASetup = () => {
    ...
    const { is2FASetup, formData: { email, tenantId } } = useAppSelector(state => state.auth.login);
    const { qrCodeBase64, secretCode, session } = useAppSelector(state => state.auth.reInitiateMFASetup);
    const { isLoading: isLoadingReInitiateMFASetup } = useLoading(API_ENDPOINTS.auth.reInitiateMFASetup);
    ...
    return (
        <div className={`custom-scrollbar h-screen pb-10 flex flex-col items-center bg-linear-270 from-[#FFFFFF] to-primary-200 relative ${"overflow-auto"}`}>
            ...
            {(isLoadingReInitiateMFASetup || isLoadingVerifyMFA) && <Loader
                className={`fixed top-0 left-0 z-998 ${isLoadingReInitiateMFASetup ? "backdrop-blur-[15px] z-999" : "backdrop-blur-[3px]"}`}
                title={isLoadingVerifyMFA ? 'Verifying Your Code' : 'Re-initializing MFA Setup'}
                description={isLoadingVerifyMFA ? 'We\'re confirming your authentication code. This will just take a moment...' : 'We\'re re-initializing your MFA setup. This will just take a moment...'} />}
        </div>
    );
};
