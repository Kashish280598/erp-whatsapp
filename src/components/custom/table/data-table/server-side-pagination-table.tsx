...
export function ServerSidePaginationTable<TData, TValue>({
  columns,
  data,
  tableToolbar,
  fetchData,
  ...
  React.useEffect(() => {
    // Only update localStorage if we have valid pagination state
  ...
  React.useEffect(() => {
    // Only fetch data if we have valid pagination state
    if (pageIndex >= 0 && pageSize > 0) {
      handleFetchData();
    }
