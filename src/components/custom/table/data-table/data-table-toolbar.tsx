...
export function DataTableToolbar<TData>({
  ...
  const [selectedFilter, setSelectedFilter] = useState<string | null>(
    tableToolbar?.filterOptions?.[0]?.field || null
  );
  ...
  useEffect(() => {
    if (isFilterOpen) {
      ...
      tableToolbar?.filterOptions?.forEach((filter) => {
        ...
        currentFilters[filter?.field || ''] = values;
      });
  ...
  const getFilterLabel = useCallback((field: string, value: string) => {
    const option = tableToolbar?.filterOptions?.find(opt => opt.field === field)
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          {enableFilter && (
            <div className="flex items-center gap-2 ml-auto">
                      ...
                      <div className="flex flex-col gap-1 w-[200px] mr-2 custom-scrollbar overflow-auto ">
                        {tableToolbar?.filterOptions?.map((filter) => (
                          <FilterCategory
                            key={filter?.field || ''}
                            filter={filter}
                            isSelected={selectedFilter === filter?.field}
                            selectedValues={getSelectedValues(filter?.field || '')}
                            onClick={() => setSelectedFilter(filter?.field || '')}
                          />
                        ))}
                      </div>

                      {/* Right Panel - Filter Options */}
                      <div className="flex-1 px-3 pr-0 w-[235px] h-full flex flex-col justify-between border-l-1 border-neutral-200">
                        {selectedFilter && (
                          <>
                            ...
                            <div className="flex justify-end gap-2 pt-4 sticky bottom-0 bg-white">
                              ...
                              <Button
                                ...
                                size="sm"
                                className={`bg-primary hover:bg-primary-400  ${!hasFilterChanges ? 'opacity-50 cursor-not-allowed' : ''
                                  }`}
              ...
              {enableCustomizeColumns && <DataTableFilterOptions table={table} />}
            </div>
          )}
        </div>
      </div>
